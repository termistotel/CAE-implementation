...
with graph1.as_default():
	...
	with tf.variable_scope("conv1"):
		l = tf.layers.conv2d(x, filternum[0], (5,5), (1,1), padding="SAME", activation=tf.nn.relu, kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=lambdad))
		l = tf.layers.conv2d(l, filternum[0], (5,5), (1,1), padding="SAME", activation=tf.nn.relu, kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=lambdad))
	...
	with tf.variable_scope("upscale3"):
		...
		l = tf.layers.conv2d(u, filternum[0], (5,5), (1,1), padding="SAME", activation=tf.nn.relu, kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=lambdad))

	with tf.variable_scope("upscale4"):
		u = tf.image.resize_images(l, size=tf.shape(x)[1:3])
		out = tf.layers.conv2d(u, 3, (7,7), (1,1), padding="SAME", activation=tf.nn.sigmoid)


	loss = tf.losses.mean_squared_error(x, out) + tf.losses.get_regularization_loss()
	optimizer = tf.train.AdamOptimizer(learning_rate=tf.train.exponential_decay(alfa,gs,10000,0.1))
	...
	with tf.name_scope("loss_data"):
		...
		logTestLossSummary = tf.summary.scalar("TestLossLog", testLoss)
		logTrainLossSummary = tf.summary.scalar("TrainLossLog", trainLoss)
